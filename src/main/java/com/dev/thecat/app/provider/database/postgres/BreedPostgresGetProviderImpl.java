...
public class BreedPostgresGetProviderImpl implements
    ...
    public List<BreedEntity> getAll() {
        return this.breedRepository.findAll().stream().map(BreedTable::toDomain).collect(Collectors.toList());
    }
    ...
    public List<BreedEntity> getByOriginId(UUID id) {
        return this.breedRepository.findByOriginId(id).stream().map(BreedTable::toDomain).collect(Collectors.toList());
    }

...
public class BreedEndpointTest {
    ...
    @Before
    public void setUp() {
        final BreedEndpoint controller = new BreedEndpoint(this.breedGetAllUseCase, this.breedGetByIdUseCase);
    ...
    public void getByIdDataNotFoundTest() {
        when(breedGetByIdUseCase.execute(any())).thenThrow(new DataNotFoundException("Breed not found"));
        this.mockMvc.perform(
                MockMvcRequestBuilders
                        .get("/v1/breeds/" + UUID.randomUUID())
                        .contentType(MediaType.APPLICATION_JSON))

...
public class BreedCreateContextDataUseCaseImpl implements BreedCreateContextDataUseCase {
  ...
  public BreedCreateContextDataUseCaseImpl(
      ...
      final BreedGetByIntegrationIdProvider breedGetByIntegrationIdProvider,
      @Qualifier("originPostgresGetProvider")
      final OriginGetByIntegrationIdProvider originGetByIntegrationIdProvider,
      @Qualifier("imagePostgresGetProvider")
      final ImageGetByIntegrationIdProvider imageGetByIntegrationIdProvider,
      @Qualifier("temperamentPostgresGetProvider")
      final TemperamentGetByIntegrationIdProvider temperamentGetByIntegrationIdProvider,
  ...
  public void execute() {
    ...
    for (int i = 0; i < breedEntityList.size(); i++) {
      ...
      var breedExist =
          this.breedGetByIntegrationIdProvider.getByIntegrationId(breedEntity.getIntegrationId());
      ...
      if (breedExist == null) {
        ...
        for (int j = 0; j < breedEntityList.get(i).getImages().size(); j++) {
          ...
          var imageExist =
              this.imageGetByIntegrationIdProvider.getByIntegrationId(image.getIntegrationId());
          if (imageExist != null) {
        ...
        for (int k = 0; k < breedEntityList.get(i).getTemperaments().size(); k++) {
          ...
          var temperamentExist = this.temperamentGetByIntegrationIdProvider
              .getByIntegrationId(temperament.getIntegrationId());

...
public class TemperamentEndpointTest {
    ...
    @Before
    public void setUp() {
        final TemperamentEndpoint controller = new TemperamentEndpoint(this.breedGetByTemperamentIdUseCase);
    ...
    public void getBreedByTemperamentIdTest() {
        when(breedGetByTemperamentIdUseCase.execute(any())).thenReturn(Collections.singletonList(BreedEntitySeeder.getBreedEntity()));
    ...
    public void getBreedByTemperamentIdDataNotFoundTest() {
        when(breedGetByTemperamentIdUseCase.execute(any())).thenThrow(new DataNotFoundException("Breed not found"));
        this.mockMvc.perform(
                MockMvcRequestBuilders
                        .get("/v1/temperaments/" + UUID.randomUUID() + "/breeds")
                        .contentType(MediaType.APPLICATION_JSON))

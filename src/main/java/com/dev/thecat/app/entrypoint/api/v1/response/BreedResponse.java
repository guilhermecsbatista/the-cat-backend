...
public class BreedResponse {
    ...
    public static BreedResponse fromDomain(final BreedEntity breed) {
        return BreedResponse.builder()
                ...
                .temperaments(breed.getTemperaments().stream().map(TemperamentResponse::fromDomain).collect(Collectors.toList()))
                .images(breed.getImages().stream().map(ImageResponse::fromDomain).collect(Collectors.toList()))
                .build();
    }
}
